...
def calc_holonomic_heuristic_with_obstacle(node, ox, oy, reso, rr):
    ...
    print("[HEURISTIC] Calculating parameters...")
    P, obsmap = calc_parameters(ox, oy, reso, rr)
    print("[HEURISTIC] Parameters calculated!")
    ...
    i = 0
    while True:
        # On every 5th iteration, print the iteration number
        i += 1
        if i % 100 == 0:
            print("[HEURISTIC] Heuristic Calculation Iteration:", i)
...
def calc_obsmap(ox, oy, rr, P):
    print(f"[OBSMAP] Calculating obstacle map with resolution {P.reso}, robot radius {rr}, and xw {P.xw}, yw {P.yw}...")
    obsmap = [[False for _ in range(P.yw)] for _ in range(P.xw)]

    for x in tqdm(range(P.xw), "Calculating obstacle map..."):
        xx = x + P.minx
        for y in range(P.yw):
            yy = y + P.miny
